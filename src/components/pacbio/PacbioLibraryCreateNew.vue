...
const provider = async () => {
  try {
    ...
    if (!success) {
      showAlert('Failed to find tags in Traction' + errors.length > 0 ? errors[0] : '', 'danger')
    }
  } catch (error) {
    showAlert('Failed to find tags in Traction' + error.message, 'danger')
  }
}
...
const createLibrary = async () => {
  ...
  if (success) {
    ...
    showAlert(
      'Failed to create library in Traction: ' + errors.length > 0 ? errors[0] : '',
      'danger',
    )
  }
}
