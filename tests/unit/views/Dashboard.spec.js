  ...
  describe('for pacbio', () => {
    ...
    describe('route buttons', () => {
      ...
      it('will have a plates button', () => {
        let platesButton = box.findAll('button').at(1)
        expect(platesButton.text()).toEqual('Plates')
        platesButton.trigger('click')
        expect(wrapper.vm.$route.path).toBe('/pacbio/plates')
      })
      it('will have a samples button', () => {
        let samplesButton = box.findAll('button').at(2)
        expect(samplesButton.text()).toEqual('Samples')
        samplesButton.trigger('click')
        expect(wrapper.vm.$route.path).toBe('/pacbio/samples')
      })
      it('will have a libraries button', () => {
        let librariesButton = box.findAll('button').at(3)
        expect(librariesButton.text()).toEqual('Libraries')
        librariesButton.trigger('click')
        expect(wrapper.vm.$route.path).toBe('/pacbio/libraries')
      })
      it('will have a runs button', () => {
        let runsButton = box.findAll('button').at(4)
        expect(runsButton.text()).toEqual('Runs')
        runsButton.trigger('click')
