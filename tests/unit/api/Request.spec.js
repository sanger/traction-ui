...
describe('Request', () => {
    ...
    describe('execute', () => {
      describe('get', () => {
        ...
        it('resolves promise to an appropriate response if successful', async () => {
          mockResponse = {
            status: 200,
            data: { data: [{ id: 1, attributes: { name: 'sample1', species: 'dog' } }] },
          }
        ...
        it('rejects promise to an appropriate response if there is an error', async () => {
          mockResponse = {
            status: 422,
            data: { errors: { name: ['error message1'], species: ['error message2'] } },
          }
  ...
  describe('build query', () => {
    ...
    it('with filter only', () => {
      ...
      wrapper = mount(cmp, { propsData: props })
      request = wrapper.vm
      expect(request.buildQuery({ filter: undefined })).toEqual(
        '?filter[type]=long_read&filter[state]=pending',
      )
    })
    ...
    it('with filter and includes', () => {
      ...
      wrapper = mount(cmp, { propsData: props })
      request = wrapper.vm
      expect(request.buildQuery({ filter: undefined, include: undefined })).toEqual(
        '?filter[type]=long_read&filter[state]=pending&include=samples.sample_metadata',
      )
    })
    ...
    it('with dynamic filter', () => {
      ...
      wrapper = mount(cmp, { propsData: props })
      request = wrapper.vm
      expect(request.buildQuery({ filter: { see: 'you_there', from: 'here' } })).toEqual(
        '?filter[see]=you_there&filter[from]=here',
      )
    })
    ...
    it('with dynamic include', () => {
      ...
      wrapper = mount(cmp, { propsData: props })
      request = wrapper.vm
      expect(request.buildQuery({ include: 'snap.crackle.pop' })).toEqual(
        '?include=snap.crackle.pop',
      )
    })
    ...
    it('with dynamic filter and includes', () => {
      ...
      expect(
        request.buildQuery({
          filter: { see: 'you_there', from: 'here' },
          include: 'snap.crackle.pop',
        }),
      ).toEqual('?filter[see]=you_there&filter[from]=here&include=snap.crackle.pop')
    })
    ...
    it('with a get', () => {
      ...
      expect(request.api.get).toBeCalledWith(
        `${request.resource}${request.buildQuery({
          filter: { type: 'long_read', state: 'pending' },
          include: 'snap.crackle.pop',
        })}`,
      )
    ...
    describe('with a find', () => {
      ...
      it('include in props', () => {
        ...
        request.find(1)
        expect(request.api.get).toBeCalledWith(
          `${request.resource}/1${request.buildQuery({ include: 'snap.crackle.pop' })}`,
        )
      })
      ...
      it('dynamic include', () => {
        ...
        expect(request.api.get).toBeCalledWith(
          `${request.resource}/1${request.buildQuery({ include: 'the.real.thing' })}`,
        )
      })
    })
  })
