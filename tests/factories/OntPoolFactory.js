...
/**
 * Factory for generating ONT pool test data in various formats for unit tests.
 *
 * Provides methods to retrieve all pools, a single pool, a specific count of pools, a pool by ID,
 * or a pool with detailed library/request/tag information. The returned objects are formatted for use
 * in Pinia stores and include both the raw API data and a `storeData` property for store state simulation.
 *
 * Methods:
 * - all(): Returns all pool records with store data for multiple pools.
 * - single(): Returns a single pool record with store data for a single pool.
 * - count(n): Returns `n` pool records (n > 1) with store data for multiple pools.
 *   Throws an error if n == 1 (use single() instead).
 * - id(poolId): Returns a pool record by ID with store data for a single pool.
 * - withDetails(id): Returns a pool record by ID, including a `details` property with an array of
 *   library details (sample_name, group_id) for each library in the pool.
 *
 * @example
 * // Get all pools
 * const allPools = OntPoolFactory.all()
 *
 * // Get a single pool
 * const singlePool = OntPoolFactory.single()
 *
 * // Get 3 pools
 * const threePools = OntPoolFactory.count(3)
 *
 * // Get pool by ID
 * const poolById = OntPoolFactory.id('2')
 *
 * // Get pool with detailed library info
 * const poolWithDetails = OntPoolFactory.withDetails('2')
 * // poolWithDetails.storeData.pool.details = [
 * //   { sample_name: 'Sample X', group_id: 'NB01' },
 * //   ...
 * // ]
 */
const OntPoolFactory = {
  all: () => {
    return {
      ...BaseFactory(data),
      storeData: createStoreDataForMultiplePools(data),
    }
  },
  single: () => {
  ...
  count: (n) => {
    ...
    const storeData = createStoreDataForMultiplePools(foundData)
    return {
      ...BaseFactory(foundData),
      storeData,
    }
