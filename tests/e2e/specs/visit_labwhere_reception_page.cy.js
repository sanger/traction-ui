...
describe('Labware Reception page', () => {
  ...
  it('exhausts samples when scanning to a destroyed location ', () => {
    ...
    cy.wrap(pacbioLibraryFactory).as('pacbioLibraryFactory')
      ...
      cy.intercept('/v1/pacbio/libraries?filter[barcode]=barcode1,barcode2&include=request,tag,tube', {
        statusCode: 200,
    ...
    cy.wait('@storeBarcodes').its('response.statusCode').should('eq', 201)
    cy.contains('barcode1, barcode2 successfully stored in location123 and sample volumes have been exhausted for 1 library')
  })
